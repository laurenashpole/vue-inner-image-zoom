<template>
  <figure class="iiz" v-bind:class="{ [className]: className, 'iis--drag': this.currentMoveType === 'drag' }" v-on="{ touchstart: handleTouchStart, click: handleClick, mouseenter: isTouch ? () => {} : handleMouseEnter, mousemove: currentMoveType === 'drag' || !isZoomed ? () => {} : handleMouseMove, mouseleave: isTouch ? () => {} : handleMouseLeave }" ref="img">
    <template v-if="validSources">
      <picture>
        <source v-for="(source, i) in validSources" :key="i" :srcSet="source.srcSet" :sizes="source.sizes" :media="source.media" :type="source.type" />
        <img class="iiz__img" v-bind:class="{ 'iiz__img--invisible': isZoomed }" v-bind:style="{ transition: `linear 0ms opacity ${isZoomed ? fadeDuration : 0}ms, linear 0ms visibility ${isZoomed ? fadeDuration : 0}ms` }" :src="src" :srcSet="srcSet" :sizes="sizes" :alt="alt" />
      </picture>
    </template>
    ...
    <template v-if="isActive">
      <img class="iiz__zoom-img" v-bind:class="{ 'iiz__zoom-img--visible': isZoomed }" v-bind:style="{ top: `${top}px`, left: `${left}px`, transition: `linear ${this.isFullscreen ? 0 : fadeDuration}ms opacity, linear ${this.isFullscreen ? 0 : fadeDuration}ms visibility` }" v-on="{ load: handleLoad, touchstart: handleDragStart, touchend: handleDragEnd, mousedown: handleDragStart, mouseup: handleDragEnd }" :src="zoomSrc || src" alt="" />

      <button v-if="this.isTouch" class="iiz__btn iiz__close" v-bind:class="{ 'iiz__close--visible': isZoomed }" v-bind:style="{transition: `linear ${this.isFullscreen ? 0 : fadeDuration}ms opacity, linear ${this.isFullscreen ? 0 : fadeDuration}ms visibility` }" v-on:click="handleClose" aria-label="Zoom Out" />
    </template>
...
export default {
  ...
  methods: {
    ...
    handleTouchStart () {
      this.isFullscreen = this.fullscreenOnMobile && window.matchMedia && window.matchMedia(`(max-width: ${this.mobileBreakpoint}px)`).matches;
      this.isTouch = true;
    ...
    handleDragStart (e) {
      this.imgProps.offsets = this.getOffsets((e.pageX || e.changedTouches[0].pageX), (e.pageY || e.changedTouches[0].pageY), this.imgProps.zoomImg.offsetLeft, this.imgProps.zoomImg.offsetTop);
      this.imgProps.zoomImg.addEventListener(this.isTouch ? 'touchmove' : 'mousemove', this.handleDragMove, { passive: false });
    ...
    initialMove (pageX, pageY) {
      this.imgProps.offsets = this.getOffsets(window.pageXOffset, window.pageYOffset, -this.imgProps.bounds.left, -this.imgProps.bounds.top);
